...
export function FoodLogger() {
  ...
  const [foodItems, setFoodItems] = useState(initialFoodItems); // Use state for food items
  ...
  const handleAddCustomFood = () => {
    ...
    setFoodItems((prevItems) => [...prevItems, customFood]);
            ...
            {showAddCustom && (
              <div className="mb-6 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {Object.keys(newFoodItem).map((key) => (
                    <input
                      key={key}
                      type={key === "calories" || key === "protein" || key === "carbs" || key === "fats" ? "number" : "text"}
                      placeholder={key.charAt(0).toUpperCase() + key.slice(1)}
                      value={newFoodItem[key]}
                      onChange={(e) => setNewFoodItem({ ...newFoodItem, [key]: e.target.value })}
              ...
              {meals.map((meal) => (
                <div key={meal} className="space-y-2">
                  ...
                  >
                    ...
                    <ChevronDown className={`w-5 h-5 transform transition-transform ${expandedMeal === meal ? 'rotate-180' : ''}`} />
                  </button>
