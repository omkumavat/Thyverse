...
const NutriNavigation = () => {
  ...
  return (
    <div className="flex">
      {/* Sidebar for Desktop */}
      <aside className="hidden md:flex flex-col w-64 h-screen bg-white dark:bg-gray-800 shadow-lg p-4 fixed top-0 left-0">
      ...
      {isMenuOpen && (
        ...
        >
          <aside className="w-64 h-full bg-white dark:bg-gray-800 shadow-lg p-4 fixed top-0 left-0">
            <div className="text-2xl font-bold text-primary mb-6">ThyVerse</div>
            <nav className="space-y-2">
              {navItems.map(({ id, icon: Icon, label }) => (
                <button
      ...
      {/* Content area */}
      <div className="md:ml-64 w-full p-6 overflow-auto">{renderTabContent()}</div>
    </div>
  );
};
